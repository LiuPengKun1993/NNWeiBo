...
protocol NNPhotoBrowserCellDelegate : NSObjectProtocol
{
    func NNPhotoBrowserCellDidClose(cell: NNPhotoBrowserCell)
}

class NNPhotoBrowserCell: UICollectionViewCell {
    
    
    weak var photoBrowserCellDelegate : NNPhotoBrowserCellDelegate?
    var imageURL: NSURL?
        {
        didSet{
            // 1.重置属性
    ...
    private func setupUI()
    {
        ...
        // 4.监听图片的点击
        let tap = UITapGestureRecognizer(target: self, action: #selector(NNPhotoBrowserCell.close))
    ...
    func close()
    {
        photoBrowserCellDelegate?.NNPhotoBrowserCellDidClose(self)
    }
...
extension NNPhotoBrowserCell: UIScrollViewDelegate
{
    // 告诉系统需要缩放哪个控件
